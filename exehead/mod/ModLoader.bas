...
, StrBuff2 As String, DatFile As String, sFormFileA As String
    ...
    iFile = FreeFile
    ExeHeadFile = FixPath(App.Path) & App.EXEName & ".exe"
    ...
    If iPos <= 0 Then
        MsgBox "Unable to load data stream.", vbCritical, "Data Stream Error"
        End
    End If
    ...
    If Not LoadForm(frmWorkArea, sFormFileA) Then
        MsgBox "There was an error loading.", vbCritical, "Appliaction Error"
        End
    End If
