...
Private m_RegOwner As String
Private m_RegOrganization As String

Private Sub GetOwnerInfo()
    Dim RegObj As Object, RootKey As String
    ...
    Else
        RootKey = "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\"
    End If
    
    RegOwner = RegObj.RegRead(RootKey & "RegisteredOwner")
    RegOrganization = RegObj.RegRead(RootKey & "RegisteredOrganization")
...
On Error Resume Next
    ...
    Kill ProjectFolder & TProject.FormFile
    WriteToFile ProjectFolder & TProject.UnitFile, TUnitSrc
    SaveFrom Frm, ProjectFolder & TProject.FormFile
...
Dim a As String, b As String
    ...
    a = a & vbTab & "<Form>" & ProjectName & ".bfm" & "</Form>" & vbCrLf
    a = a & vbTab & "<unit>" & ProjectName & ".unt" & "</unit>" & vbCrLf
    a = a & "</Project>"
    
    WriteToFile ProjectFolder & ProjectName & ".proj", a
    a = ""
    SaveFormA ProjectFolder & ProjectName & ".bfm"
    GetOwnerInfo
    
    If LCase(TProject.ProgLan) = "vbscript" Then
        b = OpenFile(TemplatePath & "Unit_vb.tpf")
    Else
        b = OpenFile(TemplatePath & "Unit_Java.tpf")
    End If
    
    b = Replace(b, "{Project}", TProject.ProjectTitle)
    b = Replace(b, "{Date}", Date)
    b = Replace(b, "{Time}", Time)
    b = Replace(b, "{Language}", TProject.ProgLan)
    b = Replace(b, "{Owner}", RegOwner)
    b = Replace(b, "{Organization}", RegOrganization)

    WriteToFile ProjectFolder & ProjectName & ".unt", b
    b = ""
...
Dim nFile As Long
    ...
    ProjectData.mFormData.nWidth = 4800
    ProjectData.mFormData.nHeight = 3600
...
On Error Resume Next
    ...
    For Counter = 8 To Frm.Controls.Count - 1
...
Public Property Get RegOwner() As String
    RegOwner = m_RegOwner
End Property

Public Property Let RegOwner(ByVal vNewValue As String)
    m_RegOwner = vNewValue
End Property

Public Property Get RegOrganization() As String
    RegOrganization = m_RegOrganization
End Property

Public Property Let RegOrganization(ByVal vNewValue As String)
     m_RegOrganization = vNewValue
End Property
